...
export default function CreatePrescriptionPage() {
        ...
        if (appointmentId) {
          ...
          if (appointmentId.match(/^app([1-9]|10)$/)) {
            ...
            if (data.length > 0) {
              ...
              const normalized = {
                ...data[0],
                medicines: (data[0].medicines || []).map((m: any) => {
          ...
          // For app11+: Use Firebase
            ...
            if (!snapshot.empty) {
              ...
              const meds = (data.medicines || [emptyMedicine()]).map((m: any) => {
                const freq: Slot[] = Array.isArray(m.frequency) ? m.frequency : (m.frequency ? [m.frequency] : []);
  ...
  const updateMedicine = (index: number, field: keyof Omit<Medicine, 'frequency' | 'quantities'>, value: any) => {
    setPrescription(prev => {
      const newMedicines = prev.medicines.slice();
